...
export default function Dashboard() {
  ...
  const [quote, setQuote] = useState(() => {
    try {
      ...
      return cached
        ? JSON.parse(cached)
        : { text: "", author: "", category: "", type: "" };
    } catch {
      return { text: "", author: "", category: "", type: "" };
    }
  });
  ...
  const fetchQuote = useCallback(() => {
    fetch(`${BACKEND_URL}/api/quotes/random?t=${Date.now()}`)
      ...
      .then((data) => {
        setQuote(data);
        try {
          localStorage.setItem("tenx_quote_cache", JSON.stringify(data));
        } catch {}
      })
      .catch(() => {
        const cached = localStorage.getItem("tenx_quote_cache");
  ...
  const calcElapsed = useCallback(() => {
    if (swState.isRunning && swState.startTimestamp) {
      return (
        swState.accumulatedSeconds +
        Math.floor((Date.now() - swState.startTimestamp) / 1000)
      );
    }
  ...
  const todayStudyMin = studySessions
    .filter((s) => s.date === today)
    .reduce((s, x) => s + x.totalMinutes, 0);

  let todayCurrTotal = 0,
    todayCurrDone = 0;
  courses.forEach((c) =>
    c.topics?.forEach((t) => {
  ...
  const fullyCompletedPapers = researchPapers.filter(
    (p) => p.completionPercentage >= 100,
  ).length;
  const todayPapersUpdated = researchPapers.filter(
    (p) => p.lastUpdated === today,
  ).length;
  ...
  const overallProgression =
    totalCurriculumItems > 0
      ...
      : 0;
  const todayProgression =
    todayCurrTotal > 0 ? Math.round((todayCurrDone / todayCurrTotal) * 100) : 0;
  ...
  const statData =
    statFilter === "today"
      ? {
          ...
          curriculum:
            todayCurrTotal === 0 ? "0/0" : `${todayCurrDone}/${todayCurrTotal}`,
  ...
  const milestones = useMemo(() => {
    ...
    const list = [
      ...
      },
      {
        id: "study_10",
        icon: <Clock size={18} />,
        label: "10 Hours Study",
        current: totalStudyHrs,
        goal: 10,
        color: "#34d399",
        tier: "bronze",
      },
      ...
      },
      {
        ...
        label: "20 Topics Done",
        current: totalCurriculum,
        goal: 20,
        color: "#22d3ee",
        tier: "bronze",
      },
  ...
  const taskCompData = days.map(
    (d) => dailyTasks.filter((t) => t.date === d && t.completed).length,
  );
  ...
  const currChartData = days.map((d) => {
    ...
    courses.forEach((co) =>
      co.topics?.forEach((t) => {
        ...
        t.subtopics?.forEach((s) => {
          if (s.completed && s.completedDate === d) c++;
        });
      }),
    );
    return c;
  });
  const paperChartData = days.map(
    (d) => researchPapers.filter((p) => p.lastUpdated === d).length,
  );
  ...
  return (
    <div className="dashboard-page">
      ...
      <div className="dash-top-row">
          ...
          <div className="dash-quote-hero-content">
            ...
            <div className="dash-quote-hero-body">
              <div className="dash-quote-hero-label">
                <Quote size={14} />{" "}
                {quote.type === "fact" ? "AI/ML Fact" : "Daily Inspiration"}
                {quote.category && (
                  <span className="dash-quote-cat">{quote.category}</span>
                )}
                {quote.source === "groq_ai" && (
                  <span className="dash-quote-ai-badge">✨ AI Generated</span>
                )}
              </div>
              {quote.text ? (
                <>
                  <blockquote className="dash-quote-hero-text">
                    "{quote.text}"
                  </blockquote>
                  <cite className="dash-quote-hero-author">
                    — {quote.author}
                  </cite>
                </>
              ) : (
                <blockquote
                  className="dash-quote-hero-text"
                  style={{ opacity: 0.4 }}
                >
                  Loading inspiration from Groq AI...
                </blockquote>
              )}
            </div>
          </div>
        ...
        <div
          className="card dash-countdown-card"
          onClick={() => !cdTarget && setShowCdInput(true)}
        >
          <div className="dash-cd-icon">
            <Hourglass size={22} color="var(--accent-primary)" />
          </div>
          {cdTarget && !cdRemaining.expired ? (
            <div className="dash-cd-display">
              ...
              <div className="dash-cd-segment">
                <span className="dash-cd-num">
                  {String(cdRemaining.hours).padStart(2, "0")}
                </span>
              ...
              <div className="dash-cd-segment">
                <span className="dash-cd-num">
                  {String(cdRemaining.minutes).padStart(2, "0")}
                </span>
              ...
              <div className="dash-cd-segment">
                <span className="dash-cd-num">
                  {String(cdRemaining.seconds).padStart(2, "0")}
                </span>
            ...
            <div className="dash-cd-display">
              <span className="dash-cd-placeholder">
                {cdTarget ? "⏰ Expired!" : "Set Target Date"}
              </span>
            </div>
          )}
          <div className="dash-cd-bottom-row">
            ...
            <div className="dash-cd-actions">
              ...
              {cdTarget && (
                <button
                  className="btn-micro btn-micro-ghost"
                  onClick={(e) => {
                    e.stopPropagation();
                    setCdTarget("");
                  }}
      ...
      <div className="card dash-stat-unified">
        <div className="dash-stat-header">
          ...
          <div className="tabs">
            <button
              className={`tab ${statFilter === "today" ? "active" : ""}`}
              onClick={() => setStatFilter("today")}
            >
              Today
            </button>
            <button
              className={`tab ${statFilter === "overall" ? "active" : ""}`}
              onClick={() => setStatFilter("overall")}
            >
              Overall
            </button>
          </div>
        ...
        <div className="dash-stat-row">
          <div className="dash-stat-item dash-stat-tasks">
            <div
              ...
              style={{
                background:
                  "linear-gradient(135deg, rgba(96,165,250,0.2), rgba(99,102,241,0.15))",
              }}
            >
              <CheckCircle size={20} color="#60a5fa" />
            </div>
            <div>
              <div className="dash-stat-label">Daily Tasks</div>
              <div className="dash-stat-value">{statData.tasks}</div>
            </div>
          </div>
          <div className="dash-stat-item dash-stat-study">
            ...
            <div>
              <div className="dash-stat-label">Study Hours</div>
              <div className="dash-stat-value">{statData.study}</div>
            </div>
          </div>
          <div className="dash-stat-item dash-stat-curriculum">
            ...
            <div>
              <div className="dash-stat-label">Curriculum</div>
              <div className="dash-stat-value">{statData.curriculum}</div>
            </div>
          </div>
          <div className="dash-stat-item dash-stat-progression">
            ...
            <div>
              <div className="dash-stat-label">Progression</div>
              <div className="dash-stat-value">{statData.progression}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {newsHeadlines.length > 0 && (
        <div className="card dash-news-card">
          <h4 className="dash-news-title">
            <Newspaper size={16} /> Trending News
          </h4>
      ...
      <div className="dash-cal-streak-row">
        <div className="card dash-cal-card">
          ...
          >
            <Calendar size={20} color="var(--accent-primary)" /> Activity
            Heatmap
          </h4>
          <Heatmap
          ...
          <div className="dash-right-row-3">
            ...
            <div className="card dash-mini-card dash-streak-card">
              ...
              <Flame
                size={24}
                color="#f59e0b"
                className={streak.count > 0 ? "dash-fire-animate" : ""}
              />
            ...
            <div className="card dash-mini-card dash-papers-card">
              ...
              <div className="dash-mini-value">
                {fullyCompletedPapers}/{researchPapers.length}
              </div>
              <div className="dash-mini-label">Papers 100%</div>
              {fullyCompletedPapers > 0 && (
                <div className="dash-mini-badge">📄</div>
              )}
            </div>
            ...
            <div className="card dash-mini-card dash-stopwatch-mini">
              ...
              <div className="dash-mini-value" style={{ fontSize: "1.1rem" }}>
                {formatSeconds(
                  swState.isRunning ? swDisplay : swState.accumulatedSeconds,
                )}
              </div>
              <div className="dash-sw-actions">
                <button
                  className={`btn-micro ${swState.isRunning ? "btn-micro-danger" : "btn-micro-primary"}`}
                  onClick={swToggle}
                >
          ...
          <div className="dash-right-row-2">
            ...
            <div className="card dash-snapshot-card">
              <div className="dash-snapshot-header">
                <h4 className="dash-snapshot-title">
                  ...
                  {todayTasks.map((t) => (
                    <li key={t.id} className={t.completed ? "done" : ""}>
                      <input
                        ...
                        checked={t.completed}
                        onChange={() =>
                          updateDailyTask(t.id, { completed: !t.completed })
                        }
                      ...
                      {editingTask === t.id ? (
                        <input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && handleSaveEdit(t.id)
                          }
            ...
            <div className="card dash-snapshot-card">
              <div className="dash-snapshot-header">
                <h4 className="dash-snapshot-title">
                  <BookOpen size={16} /> Today's Course Topics
                </h4>
              </div>
              {todayCourseItems.length === 0 ? (
                <p className="dash-snapshot-empty">
        ...
        <div className="dash-charts-header">
          <div className="dash-chart-tabs">
            {[
              ...
              <button
                key={t.key}
                className={`dash-chart-tab ${activeChart === t.key ? "active" : ""}`}
          ...
          <div className="tabs">
            <button
              className={`tab ${chartRange === "7" ? "active" : ""}`}
              onClick={() => setChartRange("7")}
            >
              7 Days
            </button>
            <button
              className={`tab ${chartRange === "30" ? "active" : ""}`}
      ...
      <div className="card dash-milestones-card">
        ...
        <div className="dash-milestones-grid">
          {milestones.map((m) => (
            <div
              key={m.id}
              className={`dash-milestone-item ${m.completed ? "completed" : ""} tier-${m.tier}`}
            >
              ...
              <div className="dash-milestone-info">
                ...
                <div className="dash-milestone-bar">
                  <div
                    className="dash-milestone-fill"
                    style={{ width: `${m.progress}%`, background: m.color }}
                  />
                </div>
                <span className="dash-milestone-progress">
                  {m.current}/{m.goal}
                </span>
              </div>
